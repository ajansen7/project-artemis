...
5. For each promising find:
   ...
   - Save to Supabase with the score: `uv run python .claude/skills/artemis/scripts/db.py add-job --title "..." --company "..." --url "..." --description "..." --source "scout" --match-score <0-100>`
6. For interesting companies without specific openings, save as target companies:
   - `uv run python .claude/skills/artemis/scripts/db.py add-company --name "..." --domain "..." --careers-url "..." --why "..." --priority "high|medium|low"`
...
**Steps:**
1. Query pipeline: `uv run python .claude/skills/artemis/scripts/db.py list-jobs`
...
4. Ask the user what to do with each (or batch):
   - **Advance**: move to next status → `uv run python .claude/skills/artemis/scripts/db.py update-job --id "..." --status "to_review"`
   - **Not interested**: `uv run python .claude/skills/artemis/scripts/db.py update-job --id "..." --status "not_interested" --reason "..."`
   - **Delete**: `uv run python .claude/skills/artemis/scripts/db.py update-job --id "..." --status "deleted"`
...
6. Update the job in Supabase: `uv run python .claude/skills/artemis/scripts/db.py update-job --id "<job_id>" --match-score <score> --analysis-file "analysis.md"`
...
3. Look up the job/company in Supabase: `uv run python .claude/skills/artemis/scripts/db.py get-job --id "..."`
...
**Steps:**
1. Run: `uv run python .claude/skills/artemis/scripts/db.py status`
...
**Steps:**
1. **Context freshness check** (see above) — refresh cache if stale
2. Look up the job/company in Supabase: `uv run python .claude/skills/artemis/scripts/db.py get-job --id "..."`
3. Read context:
...
6. Save the generated files to Supabase by running:
 `uv run python .claude/skills/artemis/scripts/db.py save-application --id "<job_id>" --resume "applications/.../resume.md" --cover-letter "applications/.../cover_letter.md" --primer "applications/.../primer.md"`
...
3. Get all active jobs: `uv run python .claude/skills/artemis/scripts/db.py list-jobs`
...
5. Collect all changes and apply via batch:
   - `echo '<json>' | uv run python .claude/skills/artemis/scripts/db.py batch-update`
...
**Batch add** (for `/scout`):
```bash
echo '[{"title":"Senior PM","company":"Anthropic","url":"https://...","description":"Build AI safety tools","match_score":85,"source":"scout"},{"title":"PM","company":"Cursor","match_score":72}]' | uv run python .claude/skills/artemis/scripts/db.py batch-add
```

**Batch update** (for `/sync`):
```bash
echo '[{"id":"uuid-1","match_score":82},{"id":"uuid-2","status":"deleted","reason":"Posting removed"},{"id":"uuid-3","match_score":55,"status":"to_review"}]' | uv run python .claude/skills/artemis/scripts/db.py batch-update
```
