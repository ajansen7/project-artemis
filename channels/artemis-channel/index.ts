...
const mcp = new Server(
  { name: "artemis-channel", version: "1.0.0" },
  {
    ...
    instructions: [
      ...
      "   uv run python tools/db.py update-task --id <id> --status complete --output-summary '<brief summary>'",
      "   On failure: uv run python tools/db.py update-task --id <id> --status failed --error '<reason>'",
      "4. Send a brief Telegram summary:",
      "   uv run python tools/push_to_telegram.py summary --job-name '<task name>' --status success --body '<summary>'",
      "",
