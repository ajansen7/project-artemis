...
const mcp = new Server(
  { name: "artemis-webhook", version: "0.1.0" },
  {
    ...
    instructions: [
      ...
      '1. Check Supabase for any engagement_log entries with status "drafted" that need user approval.',
      "2. Check for upcoming interviews in the next 7 days (query jobs where status = 'interviewing').",
      "3. Check for blog_posts with status 'draft' or 'review' that are ready to publish.",
      "4. Format a concise summary of what the job did and any pending actions.",
      "5. Reply to the user via the Telegram or Discord channel reply tool so they see it on their phone.",
      "",
...
Bun.serve({
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    try {
      ...
      const { job, skill, status, error } = body as {
        job?: string;
        skill?: string;
        status?: string;
        error?: string;
