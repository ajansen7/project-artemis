...
The `/artemis:review` and `/artemis:sync` buttons in the top-right trigger the agent directly from the dashboard.
...
- **Match score** — 0-100, generated by the agent during `/artemis:analyze`
...
- **Action buttons:**
  ...
  - **Analyze** — run `/artemis:analyze` on this job right now
...
The Engagement tab is the approval queue for LinkedIn activity generated by the `/artemis:linkedin-scout` skill. The three buckets are:
...
The Blog tab tracks your content pipeline — from raw idea through draft, review, and published. Posts are generated by the `/artemis:blog-write` skill, aligned to your identity and voice, and targeted at themes relevant to your target companies.

Run `/artemis:blog-ideas` or `/artemis:blog-write` in Claude to generate ideas and drafts. They'll appear here for review before publishing.
...
Present on every tab, the sidebar shows your company watchlist ranked by priority. Each entry includes a brief rationale for why it's a target. The `/artemis:scout` skill uses this list to prioritize where to look for jobs.
...
|---------|-------------|
| `/artemis:scout` | Discover new job leads across target companies |
| `/artemis:sync` | Pull in jobs from LinkedIn and other sources |
| `/artemis:review` | Triage the To Review queue with the agent |
| `/artemis:analyze` | Deep-evaluate a specific job for fit and gaps |
| `/artemis:generate` | Generate application materials for a job |
| `/artemis:network` | Surface and advance networking contacts |
| `/artemis:linkedin-scout` | Browse LinkedIn via Chrome for jobs and contacts |
| `/artemis:inbox` | Scan Gmail and Calendar for job search activity |
| `/artemis:blog-write` | Generate and draft blog post ideas |
| `/artemis:status` | Get a full pipeline summary |
